...
% Visualization
visualize(TTerm, Vis) :-
	visualization(TTerm, OrigVis),
	transformTerm(OrigVis, Vis, visualization, set(pair(vis(TChild), ChildVis), visualize(TChild, ChildVis)), visualization).

% Test.....
:- dynamic loadedFile/3.
loadedFile(a, [pair(x,x)], []).
